...
export function GeminiNode({ id, data }: FlowNodeProps<GeminiData>) {
  ...
  return (
    <NodeShell data={data} accent="violet">
      <Handle id="in-prompt" type="target" position={Position.Left} className="!top-[82px] !size-3.5 !border-2 !border-white !bg-[#f59e0b]" />
      <Handle id="in-image" type="target" position={Position.Left} className="!top-[152px] !size-3.5 !border-2 !border-white !bg-[#2f80ed]" />
      ...
      <div className="space-y-2">
        <label className="block rounded-lg border border-[#ececf1] bg-[#fbfbfd] p-2">
          ...
          <textarea
            className="mt-1 h-14 w-full resize-none rounded-md border border-[#e1e1e8] bg-white p-2 text-xs outline-none"
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] text-[#777a84]">
          <div className="rounded-lg border border-[#ececf1] bg-[#fbfbfd] p-2">Image (Vision)</div>
